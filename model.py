...
class sshda(torch.nn.Module):
    def __init__(self,config, n_classes, n_domain, dates=None):
        ...
        self.encoder_s = CasualConvTran(config=config,num_classes= 64,dates=dates) #on pause num_classes=64 afin que la sortie du transformer soit en dimension 64 
                                                                                    # on va ensuite ajouter une couche pour classifier le domaine et les label
        self.encoder_t =  CasualConvTran(config=config,num_classes= 64,dates=dates)
        self.dom_class = nn.LazyLinear(n_domain)
        self.lab_class = nn.LazyLinear(n_classes)
        self.normalize = nn.BatchNorm1d(64)
        self.dropout =  nn.Dropout(0.8)
        self.relu = nn.ReLU()
    def forward(self,x_s,mask_s,x_t,mask_t):
        emb_s = self.encoder_s(x_s,mask_s)
        emb_dom_s = emb_s[:,:32]
        emb_lab_s = emb_s[:,32:]
        emb_s = self.normalize(emb_s)
        emb_s = self.relu(emb_s)
        pred_dom_s = self.dom_class(emb_s[:,:32])
        pred_lab_s = self.lab_class(emb_s[:,32:])
        emb_t = self.encoder_s(x_t,mask_t)
        emb_dom_t = emb_t[:,:32]
        emb_lab_t = emb_t[:,32:]
        emb_t = self.normalize(emb_t)
        emb_t = self.relu(emb_t)
        pred_dom_t = self.dom_class(emb_t[:,:32])
        pred_lab_t = self.lab_class(emb_t[:,32:])
        return pred_lab_s,pred_dom_s,emb_lab_s, emb_dom_s, pred_lab_t,pred_dom_t,emb_lab_t, emb_dom_t
